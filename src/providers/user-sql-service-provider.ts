...
export class UserSqlServiceProvider implements UserSqlServiceProviderInterface {
  ...
  doesTableExist(callbackMethod) {
    this.sqlite.create(this.options).then((db: SQLiteObject) => {
      db.executeSql("SELECT count(*) as recCount FROM sqlite_master WHERE name='Users'",[]).then((data) => {
  ...
  initialiseTable(callbackMethod) {
    this.sqlite.create(this.options).then((db: SQLiteObject) => {
      db.executeSql('CREATE TABLE IF NOT EXISTS Users(id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, surname TEXT, cellphone TEXT, email TEXT, province TEXT, userType TEXT, password TEXT, inSync NUMERIC)', {}).then((data) => {
  ...
  insertRecord(userModel: UserModel, callbackMethod) {
    this.sqlite.create(this.options).then((db: SQLiteObject) => {
      db.executeSql('INSERT INTO Users (name, surname, cellphone, email, province, userType, password, inSync) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
      , [
          userModel.name, 
          userModel.surname, 
          userModel.cellphone, 
  ...
  getRecord(id, callbackMethod) {
    ...
    this.sqlite.create(this.options).then((db: SQLiteObject) => {
      db.executeSql("SELECT * FROM Users WHERE id =" + id ,[]).then((data) => {
        result = [];
  ...
  deleteRecord(id: number, callbackMethod){
    this.sqlite.create(this.options).then((db: SQLiteObject) => {
      db.executeSql("DELETE FROM Users WHERE id=" + id,[]).then((data) => {
        callbackMethod({success: true, data: data});
      }, (err) => {
        callbackMethod({success: false, data: err});
      });
  ...
  syncTable(userModels: UserModel[], callbackMethod){
    this.sqlite.create(this.options).then((db: SQLiteObject) => {
      db.executeSql('DELETE FROM Users', {}).then((data) => {} , (err) => {
        callbackMethod({success: false, data: err}); 
        return;
      });
  ...
  getAllNonSyncedRecords(callbackMethod){
    ...
    this.sqlite.create(this.options).then((db: SQLiteObject) => {
      db.executeSql("SELECT * FROM Users WHERE inSync = 0",[]).then((data) => {
        result = [];
        if(data.rows.length > 0) {
            for(var i = 0; i < data.rows.length; i++) {
                result.push({
